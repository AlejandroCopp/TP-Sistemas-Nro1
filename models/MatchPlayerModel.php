...
class MatchPlayerModel {
    ...
    public function getPendingPlayersByMatchId($matchId) {
        ...
        $sql = "SELECT u.id, u.name, mp.position, mp.team FROM match_players mp JOIN users u ON mp.player_id = u.id WHERE mp.match_id = :match_id AND mp.datetime_player_declined IS NULL AND mp.datetime_player_added IS NULL";
        $stmt = $this->db->prepare($sql);
        $stmt->bindParam(':match_id', $matchId);
        $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
